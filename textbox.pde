class Textbox {
  ...
  boolean keyPressed(char KEY, int KeyCode) {
    if (selected) {
      if (KeyCode == (int)BACKSPACE) backSpace();
      else if (KeyCode == 32) addText(' '); //space key
      else if (KeyCode == (int)ENTER) return true;
      else {
        ...
        if (isKeyCapitalLetter || isKeySmallLetter || isKeyNumber) addText(KEY);
      }
    }
    return false;
  }
  ...
  boolean overBox(int x, int y) {
    if (x >= X - W/2 && x <= X + W/2) {
      if (y >= Y - H/2 && y <= Y + H/2)  return true;
    }
    return false;
  }
...
void layout() {
  ...
  Textbox message2 = new Textbox(width / 2, 300, 300, 100);
  Textbox message3 = new Textbox(width / 2, 450, 300, 100);
  textboxes.add(message);
  textboxes.add(message2);
  textboxes.add(message3);
}
